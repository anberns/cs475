...

{
...

         {
...

                 {
                        ...
                        #pragma omp parallel for reduction(+:fx), reduction(+:fy), reduction(+:fz), schedule(dynamic)
         ...
         printf("Fine / Dynamic / %s threads: %8.2lf\n", argv[1], ((double)(NUMBODIES * NUMBODIES * NUMSTEPS)/(time1 - time0)/1000000));
